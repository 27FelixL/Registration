...
class Graph:
    ...
    def compute_depth(self):
        ...
        def dfs_depth(u):
            ...
            for v in self.get_adjacent_vertices(u):
                child_depth = dfs_depth(v) + 1
                if child_depth > max_child:
                    max_child = child_depth
            self.vertices[u].depth = max_child
            return max_child
    ...
    def get_registration_plan(self):
        ...
        courses = []
        while available:
            available.sort(key=lambda i: (-self.vertices[i].depth, self.vertices[i].label))
            semester = available[:4]
            courses.append([self.vertices[i].label for i in semester])
            for u in semester:
                in_deg[u] = -1
            new_avail = []
            for u in semester:
                for v in self.get_adjacent_vertices(u):
                    ...
                    if in_deg[v] == 0:
                        new_avail.append(v)
            available = available[4:] + new_avail
        return courses
...
def main():
    ...
    for _ in range(num_edges):
        ...
        start_idx = graph.get_index(parts[0])
        end_idx = graph.get_index(parts[1])
